...
function _hybridCreateGame(k)
{
	...
	g.results={started:false,ended:false,score:0,starttime*0.01:-1,endtime*0.01:-1,startcount:-1,endcount:-1,win:false,missions:[],lost:false}; // you didn't win or loose, we don't know ANYthing..
	switch(g.k)
	{
		case "scrolling":
			// display lists!
...
function _hybridGameStart(g)
{
	if(g.results.started!=true)
	{
		...
		g.results.starttime*0.01=new Date().gettime*0.01();
...
function _hybridGameEnd(g,win)
{
	if(g.results.ended!=true)
	{
		...
		g.results.endtime*0.01=new Date().gettime*0.01();
		...
		for(c=0;c<g.lay[l].csec.length;c++) //
		{
			...
			for(i=s.o.length-1;i>=0;i--)
			{
				...
				}else
				{
					...
					{
						// if it's not there yet, rooms get added 6 to 7 time*0.01s now..
						// that's bad...
						g.rend.push(s.o[i]);
					}
				}
