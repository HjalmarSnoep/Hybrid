...
function _hybridDoPhysicsParticleStep(the_object,courseness)
{
	...
	for(i=0;i<the_object.phys.pc.length;i++)  // physics connections..
	{
...
function _hybridCreatePhysObject(vertices,init_object,clamp)
{
	...
	x_as={x:Math.cos(init_object.r*Hybrid.TO_RADIANS),y:Math.sin(init_object.r*Hybrid.TO_RADIANS),k:"axis"};
	y_as={x:x_as.y,y:-x_as.x,k:"axis"};
	phys.op=JSON.parse(JSON.stringify(vertices)); // the original hit polygon, this will be transformed by the axis defined
	// by the springbox, to create the wp of world polygon.
    // if't against this polygon that the vx,vy of the sentries of other springboxes will be tested.
	phys.wp=[]; // vp remains empty until the object has been actually placed.
...
//			{
			...
			if(len!=0)
			{
				dx=dx/len;
				dy=dy/len;
				Hybrid.debugmessage("point speed: "+dx+","+dy);
				v.dx=init_object.dr*dx; 
				v.dy=init_object.dr*dy;
			}
			
			phys.p.push(v);
			var v={x:init_object.x+vertices[i].x*ddx[0]+vertices[i].x*ddy[0],y:init_object.y+vertices[i].y*ddx[1]+vertices[i].y*ddy[1]};
			phys.wp.push(v); // // this places the object for the first time..
		}
	}
	...
	// calculate the hit-radius
	var len;
	phys.hr=0;
	for(i=3;i<phys.p.length;i++) // 3 is first normal sentry, 0=pivot, 1=x,2=y
	{
		len=_hybridDistanceBetweenPoints(phys.p[0],phys.p[i]);
		if(len>phys.hr)phys.hr=len; // try and get a hit-radius..
	}
	// now size the axis to the hit-radius.
	var axis_scale=phys.hr;
	if(axis_scale<5) axis_scale=5;
	x_as.x*=axis_scale;
	x_as.y*=axis_scale;
	y_as.x*=axis_scale;
	y_as.y*=axis_scale;
	...
	// we now know how big the rad is.
...
{
	var o={x:0,y:0,dx:0,dy:0,f:0,a:"unknown",at:false,ld:false,dd:true,r:0,dr:0,p:"static",k:"unknown",kind:"HybridGameObject",bb:{x:-15,y:-15,w:30,h:30},hs:{k:"rect",x:-10,y:-10,w:20,h:20}};   //we mean SHAPE, but actually we are using this as a sentty definition.. 
	_hybridOverrideObjectPropsWith(o,io); // override with init object.
