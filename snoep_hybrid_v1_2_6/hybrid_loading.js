...
function _hybridDoPreload(){
 ...
 if(Hybrid.capabilities.audio_preload==true)
 {
   ...
   {
		if(Hybrid.audio_manifest[i].use_tag==true)
		{
			Hybrid.audio_manifest[i].au={}; // create an au object, which will hold the actual audio!
			Hybrid.audio_manifest[i].preload=false;// no more preload!
			// create a node on lib layer!
			// lib will contain audio, bitmap-image, svg-image and other 'normal' resources
			html_string='<audio id="'+i+'">';  
			html_string+='<source src="snd_ogg/'+i+'.ogg" type="audio/ogg">';
			html_string+='<source src="snd_mp3/'+i+'.mp3" type="audio/mpeg">';
			html_string+='Your browser does not support the audio element.';
			html_string+='</audio>';
			$(html_string).appendTo("#lib");
		}else
		{
			Hybrid.audio_manifest[i].au={}; // create an au object, which will hold the actual audio!
...
function _hybridPreloadNextImage()
{
	...
	//  for(var i in Hybrid.graphics_manifest){
		if(Hybrid.graphics_manifest[i].preload){
			...
			{
//				Hybrid.debugmessage("direct dataurl found in image manifest!" +Hybrid.graphics_manifest[i].src);
				Hybrid.graphics_manifest[i].img.src=Hybrid.graphics_manifest[i].src; // keep a ref tot the img, to use with canvas.
				Hybrid.graphics_manifest[i].img.alt="dataUrl:"+i;
//				Hybrid.debugmessage("ImageManifest:" +JSON.stringify(Hybrid.graphics_manifest[i]));
				Hybrid.currentLoadingUrl="data URI"; // keep a ref tot the img, to use with canvas.
			}else
			{
				...
				Hybrid.graphics_manifest[i].img.alt=Hybrid.img_src_path+Hybrid.graphics_manifest[i].src; // alt is same as image src, handy to report failure to load?.
				Hybrid.currentLoadingUrl="data URI"; // keep a ref tot the img, to use with canvas.
			}
 ...
 Hybrid.getBoundsFromManifest=_hybridGetBoundsFromManifest;
 function _hybridGetBoundsFromManifest(i,spritename,frame)
 {
	if(typeof(spritename)==="undefined")
	{
		...
		if(typeof(Hybrid.graphics_manifest[i])=="undefined")
		{	
			Hybrid.debugmessage("WARNING: this name not found in manifest: '"+i+ "', see for yourself: "+JSON.stringify(Hybrid.graphics_manifest));
			return {x:0,y:0,w:0,h:0};
		}
		return {x:0,y:0,w:Hybrid.graphics_manifest[i].w,h:Hybrid.graphics_manifest[i].h};
	}else
	{
		Hybrid.debugmessage("this is easy enough to implement, don't know if we'll ever use it though..");
		return {x:0,y:0,w:0,h:0,name:"sorry this ain't implemented yet.."};
	}
 }
