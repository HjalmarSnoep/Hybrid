...
Hybrid.version="1.2.6"; // todo have a singular object, better libs!
...
// 1.2.6 will has a singular object, also easy lib loading, tron-support and better asset-libs!
Hybrid.copyright_year=2016;
Hybrid.copyright_url="";
Hybrid.copyright_name="";
...
Hybrid.platform.mobile=false;
var mobile_hints=["android","blackberry","iphone","ipad","ipod","opera mini","iemobile"];
var ua=navigator.userAgent.toLowerCase();
for(var i=0;i<mobile_hints.length;i++)
{
	if(ua.indexOf(mobile_hints[i])!=-1) Hybrid.platform.mobile=true; // adapt the manifest, please, so they won't load as much stuff.
}
...
function _hybridInit(){
   ...
   }else
   {
	   ...
	   Hybrid.createPage();  // creates the page and lib, this is NEW in Hybrid 6!
	   
	   Hybrid.erasePage();  // does not create the page, just erases content and sets color, also resets the listeners.
...
Hybrid.animationRequestMade=false;
Hybrid.def_pageloopFunction=null;

// we get the problem now, that the page is being called, after it unloads!
// we might want to get back to the other loop function (setInterval)
// or revisit this problem again..
// wait, I copied in the 2.4 version and it still happens.
// might be something else wrong.

function _hybridStartLoopHandler()
{
	//Hybrid.debugmessage("_hybridStartLoopHandler called ");
	if (window.requestAnimationFrame)
	{
		//Hybrid.debugmessage("requestAnimationFrame exists");
		 if(Hybrid.animationRequestMade==false)
		 {
			//Hybrid.debugmessage("no animation request have been made yet");
			Hybrid.animationRequestMade=true; // no multiple calls will be made if processor can't handle it!
			//Hybrid.debugmessage("Hybrid.animationRequestMade set to true");
			var element=document.getElementById("hybridStage");
			//Hybrid.debugmessage("element found "+element);
			window.requestAnimationFrame(_hybridRequestAnimationFrameCompatibilityFunction,element);
			//Hybrid.debugmessage("Made animation request");
		 }
	}else
	{
		Hybrid.def_pageloopFunction(); // just call it.
	}
}
function _hybridRequestAnimationFrameCompatibilityFunction()
{
	//Hybrid.debugmessage("_hybridRequestAnimationFrameCompatibilityFunction called!");
	if(Hybrid.def_pageloopFunction!=null)Hybrid.def_pageloopFunction(); // just call it. // but only if allowed.. and not just stopped a loop!
	Hybrid.animationRequestMade=false; // we can make a new request!
}
// public function to do loops
Hybrid.startLoop=_hybridStartLoop;
function _hybridStartLoop(func, interval)
{
	//Hybrid.debugmessage("Hybrid.startLoop called "+interval);
	if(Hybrid.def_pageloop!=-1)
	{
		_hybridStopLoop();
	}
	Hybrid.def_pageloopFunction=func;
	Hybrid.def_pageloop=window.setInterval(_hybridStartLoopHandler,interval);
}
...
function _hybridStopLoop()
{
	...
	Hybrid.animationRequestMade=false; // enable you to start loops again..
	Hybrid.def_pageloop=-1;
	Hybrid.def_pageloopFunction=null;
	return true;
} 
...
Hybrid.checkGraphicsManifest=_hybridCheckGraphicsManifest;
function _hybridCheckGraphicsManifest()
{
	// each time you add a sprite to the manifest, you need to call this.
	// check integrity of all spritesheets, all sprite, all frames only after first time!
	for(var img_nr in Hybrid.graphics_manifest)
	{
		if(Hybrid.graphics_manifest[img_nr].kind="sprite")
		{
			for(var label in Hybrid.graphics_manifest[img_nr].ss)
			{
				for(var f=0;f<Hybrid.graphics_manifest[img_nr].ss[label].length;f++)
				{
					...
					if(Hybrid.graphics_manifest[img_nr].ss[label][f][7]!=1)
					{
						// this will ONLY be checked once!
						Hybrid.graphics_manifest[img_nr].ss[label][f][7]=1; // indicate it has been fixed!
						var throw_warning=false;
						if(px<0){ Hybrid.graphics_manifest[img_nr].ss[label][f][0]=0;	throw_warning=true;}
						if(py<0){ Hybrid.graphics_manifest[img_nr].ss[label][f][1]=0;	throw_warning=true;}
						if((px+bw)>Hybrid.graphics_manifest[img_nr].w){ throw_warning=true; Hybrid.graphics_manifest[img_nr].ss[label][f][2]=Hybrid.graphics_manifest[img_nr].w-px-1;}
						if((py+bh)>Hybrid.graphics_manifest[img_nr].h){ throw_warning=true; Hybrid.graphics_manifest[img_nr].ss[label][f][3]=Hybrid.graphics_manifest[img_nr].h-py-1;}
						if(throw_warning)
						{
							Hybrid.debugmessage("WARNING: There is a critical vulnerability in spritesheet: "+img_nr+", sprite "+label+", frame "+f+" , some browsers like safari and Android Default browser will trip over this, so it has been fixed but might give you some unpredictable results like partially missing sprites. It might simply be, the dimensions you gave for your spritesheet are off, but it might also be an error of the spritesheet generator. Please check!");
							var px=Hybrid.graphics_manifest[img_nr].ss[label][f][0];
							var py=Hybrid.graphics_manifest[img_nr].ss[label][f][1];
							var bw=Hybrid.graphics_manifest[img_nr].ss[label][f][2];
							var bh=Hybrid.graphics_manifest[img_nr].ss[label][f][3];
							Hybrid.debugmessage("New frame: "+px+","+py+" "+(px+bw)+"x"+(py+bh)+" source:"+Hybrid.graphics_manifest[img_nr].ss[label][f]);
						}
					}
				}
			}
...
function _hybridGetCapabilities(){
 ...
 if( !(window.ActiveXObject) && "ActiveXObject" in window) is_explorer=true;
  ...
  Hybrid.platform.browser="unidentified";
	// execute this directly, so we can adapt the manifest!
 ...
 function _hybridGetDevice(){
   ...
   if (ua.match(/IEMobile/i)) Hybrid.platform.device="IEMobile";
   if (ua.match(/Opera Mini/i)) Hybrid.platform.device="Opera Mini";
