...
function _hybridCreateGame(k)
{
	...
	switch(g.k)
	{
		case "scrolling":
			...
			g.minzoom=1;
			g.maxzoom=1;
			g.cam=_hybridGameCreateGameObject(g,null,{k:"cam",a:{x:0,y:0},zoom:1,smooth:0.8}); //a is actual position. x,y are target, smooth sets transition!
...
{
	var o={x:0,y:0,dx:0,dy:0,f:0,a:"unknown",at:false,ld:false,r:0,dr:0,p:"static",k:"unknown",kind:"HybridGameObject",bb:{x:-15,y:-15,w:30,h:30},hs:{k:"rect",x:-10,y:-10,w:20,h:20}};
	_hybridOverrideObjectPropsWith(o,io);
	...
	//		a.push(o); // set directly to dynamic objects of layer, this should be done a little bit more intelligently, but ok.
		}else
		{
			...
			if(o.at)
			{
				...
				//just to be precise, we add the layer number anyway.
