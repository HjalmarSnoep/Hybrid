...
Hybrid.getPageScale=_hybridGetPageScale;
...
// function to set the Hybrid Structure.. Literally create the page struct, including lib div!
Hybrid.createPage=_hybridCreatePage;
function _hybridCreatePage()
{
	var $body = $('body');
	// deletes all layers accept top one	
	var html_string="<div id='back'  style='left: 0px; top: 0px; width: 100%; height: 100%;'>";
	html_string+="<div id='hybridStage' style='left: "+Hybrid.ox+"px; top: "+Hybrid.oy+"px;width: "+toPx(Hybrid.width)+"; height: "+toPx(Hybrid.height)+";'>";
	html_string+="</div></div>"; // deletes all layers accept top one
	// creates the LIB div.
	html_string+="<div id='lib' style='display: none;'></div>";  // this will contain audio, bitmap-image, svg-image and other 'normal' resources
	html_string+="<div id='trons' style='display: none;'></div>";  // this will contain all iframes with trons!
	$body.html(html_string);
	Hybrid.debugmessage("Created Page");
}
...
{
	$("#hybridStage").empty(); // empties page div!
	if(Hybrid.stageColor)
	{
		$("#back").css("background-color",Hybrid.stageColor);
		$('body').css("background-color",Hybrid.stageColor);
	}
	/*
	var $body = $('body');
	var html_string="<div id='back'  style='left: 0px; top: 0px; width: 100%; height: 100%;'><div id='hybridStage' style='left: "+Hybrid.ox+"px; top: "+Hybrid.oy+"px;width: "+toPx(Hybrid.width)+"; height: "+toPx(Hybrid.height)+";'></div></div>"; // deletes all layers except top one
	if(Hybrid.stageColor)
	{
