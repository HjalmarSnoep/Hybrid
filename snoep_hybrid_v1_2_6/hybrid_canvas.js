...
{
	//console.log("draw sprite"+hbc+","+img_nr+","+label+","+x+","+y+","+f+","+r+","+sx+","+sy);
	var ctx;
...
//	Hybrid.debugmessage("_hybridDrawBitmapSprite"+hbc+","+img_nr+","+label+","+x+","+y+","+f+","+r+","+sx+","+sy)
   ...
   }else
   {
    ...
    image=Hybrid.graphics_manifest[img_nr].img;
   	...
   	// there is a vulnerability here, we need to check for:
	//	http://hybridgameengine.blogspot.nl/2015/11/bugs-in-drawsprite-thanks-to-automatic.html
/*	if(Hybrid.graphics_manifest[img_nr].ss[label][f][7]!=1)
	{
		// this will ONLY be checked once!
		Hybrid.graphics_manifest[img_nr].ss[label][f][7]=1;
		var throw_warning=false;
		if(px<0){ px=0;	throw_warning=true;}
		if(py<0){ py=0;	throw_warning=true;}
		if((px+bw)>Hybrid.graphics_manifest[img_nr].w){ throw_warning=true; bw=Hybrid.graphics_manifest[img_nr].w-px-1;}
		if((py+bh)>Hybrid.graphics_manifest[img_nr].h){ throw_warning=true; bh=Hybrid.graphics_manifest[img_nr].h-py-1;}
		if(throw_warning)
		{
			Hybrid.debugmessage("There is a critical vulnerability in spritesheet: "+img_nr+", sprite "+label+", frame "+f+" , some browsers like safari and Android Default browser will trip over this, so it needs to be addressed! It has been fixed for now, so it's not a critical thing, but REALLY slows down performance. Call Hybrid.checkGraphicsManifest to check your sprites and make this unnecessary.");
			Hybrid.graphics_manifest[img_nr].ss[label][f][7]=0;
			// check again next time.
		}
	}*/
	// this is NOT necessary if only you call check after creating a manifest!
   ...
   // http://stackoverflow.com/questions/19338032/canvas-indexsizeerror-index-or-size-is-negative-or-greater-than-the-allowed-a
   //Hybrid.debugmessage("firefox drawImage"+img_nr+","+label+","+f+" -> "+px+","+py+" "+(px+bw)+"x"+(py+bh));
...
{
	Hybrid.debugmessage("_hybridDrawImage canvas,img,x,y,r,s: "+hbc+","+img_nr+","+x+","+y+","+r+","+s);
	if(r===undefined) r=0;
	if(s===undefined) s=1;
	if(x===undefined) x=0;
	if(y===undefined) y=0;
	...
	Hybrid.debugmessage("_hybridDrawImage3: "+hbc+","+img_nr+","+image.width+","+image.height);
...
function _hybridCanvasToImageManifest(hbc,label)
{
	...
	// this seems to fail on a android stock browser as of 16-11-2015.
	Hybrid.graphics_manifest[label].img.src=hbc.canvas.toDataURL();
	Hybrid.graphics_manifest[label].src='data:'+hbc.canvas.toDataURL();
	Hybrid.debugmessage("saved a new image to manifest: "+label) ;//+" => "+Hybrid.graphics_manifest[label].src);	
}
